...
def detect_intent(project, session_id, text, language_code):
      ...
      query_input = dialogflow.types.QueryInput(text=dialogflow.types.TextInput(text=text, language_code=language_code))
      response = session_client.detect_intent(session=session, query_input=query_input)

      return response.query_result.fulfillment_messages

class Bot:
      def __init__(self, homeserver, userID, token, ownerID, dialogflow_project):
            self.ownerID = ownerID
            self.userID = userID
            self.client = MatrixClient(homeserver, token, userID)
      ...
      def accept_invite(self, room_id, state):
            ...
            query_input = dialogflow.types.QueryInput(event=dialogflow.types.EventInput(name='WELCOME', language_code='en'))
      ...
      def onEvent(self, room, event):
            if event['sender'] != self.userID:
                  ...
                  if event['type'] == "m.room.message" and event['content']['msgtype'] == "m.text":
                        ...
                        response = detect_intent(self.project, room.room_id, event['content']['body'], 'en')
...
if __name__ == "__main__":
      ...
      bot = Bot(cfg['matrix']['homeserver'], cfg['matrix']['user_id'], cfg['matrix']['token'], cfg['matrix']['owner_id'], cfg['dialogflow']['project'])
      bot.run()